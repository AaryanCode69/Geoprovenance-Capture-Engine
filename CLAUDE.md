...
**Built and passing — 523 tests, `make test`, no QGIS required:**
...
- `tests/fixtures/` — **A0.3 complete.** `mock_provenance.db` (3 workflows, 16 jobs, 23 datasets, 71 relations), `mock_events.json`, `mock_ids.json`, real Shapefile + GeoPackage in `data/`, all regenerable via `make fixtures` and byte-deterministic *on one machine*. **B and C are unblocked** — see `tests/fixtures/README.md`.
...
- **Fixed 31 Aug 2026 — `w1` was missing a derivation.** "Buffer then Clip" was research doc §7.3 transcribed literally, including its omission of `final_roads.shp <- city_boundary.shp` — so the fixture asserted that editing the boundary could not affect the clip result. It plainly could, and that is the exact claim `audit.py`'s `input_unchanged` check exists to make, so the shared artefact was training every consumer on a falsehood. The other two workflows loop over every input and could not make the mistake; `w1` was a hand-written tuple. **Held pinned for one day** (`{"Buffer then Clip": 1}`) while the §3.4 step 5 notification was written, then fixed: `relations` 70 → 71, **no ids changed**, `mock_ids.json` and `mock_events.json` byte-identical, no schema change and no migration. `tests/prov/test_prov.py` now proves the inference and the recorded record agree on all three workflows with nothing left over, and `test_the_clip_result_comes_from_the_boundary_too` pins the link itself. Told to B and C in `tests/fixtures/README.md`; research doc §7.3 is left as it stands, with the departure recorded there.
...
make test-fingerprint # Person B's fingerprint layer — no QGIS needed
