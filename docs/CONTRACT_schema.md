...
| `fingerprints` | `hash_strategy` | Which measurement this row is. `file` / `schema_sample` (B's §6.4 tiered fallback — exactly one per dataset), plus the complementary `structure` / `geometry` / `attributes`. Part of the uniqueness key — see decision 4 |
...
|---|---|---|---|
| 2026-08-30 | **2** (unchanged) | **No DDL change, no migration, no fixture rebuild — `user_version` stays 2.** What changed is what gets *written* into the column the v2 key was widened for. Until now nothing produced more than one measurement per dataset, so the widened key permitted a shape nothing wrote. `hash_strategy` gains three complementary values alongside the existing `file` / `schema_sample`: **`structure`** (field names, field types and CRS), **`geometry`** (feature count and bounding box) and **`attributes`** (the attribute values themselves — for a Shapefile, the `.dbf` that the byte hash of the `.shp` never reads). They are complements, not substitutes: exactly one primary strategy is still produced per dataset, and the complementary rows only mean anything when compared against it. Motivation is RQ3 (research doc §9.1, detection accuracy), where a byte hash alone is wrong in both directions — a GeoPackage re-saved by a different SQLite build reads as changed when it is not, and an edited `.dbf` reads as unchanged when it is. The column still has **no `CHECK`**: constraining it would mean a migration per added strategy, so `geoprovenance.fingerprint.KNOWN_STRATEGIES` is the single definition and a test asserts nothing escapes it. | **Person A:** no action — the schema, the migration and the committed fixtures are untouched, and `git status` staying clean of `tests/fixtures/` is the check. **Person B:** `fingerprint_dataset(path)` returns the whole set and `store.add_fingerprints(entity_id=..., fingerprints=...)` writes it under **one** `computed_at`, which is what makes "per instant" true of the data; writing rows one at a time gives each its own timestamp. `fingerprint_file()` is unchanged in signature and in the value it returns, so nothing existing breaks. **Person C — this supersedes the 2026-08-26 note telling you to filter on `hash_strategy` yourself.** `get_latest_fingerprint(entity_id, hash_strategy)` now takes the filter, and without one it is at least deterministic (previously, with several rows sharing an instant, which row came back was arbitrary). For "has this input changed?", prefer `get_fingerprint_set(entity_id)` — newest row of each method — fed to `compare_fingerprint_sets()`, which returns `unchanged` / `resaved` / `attributes_changed` / `geometry_changed` / `schema_changed` / `changed` / `unknown` plus a plain-English `.explain()`. **Score `resaved` as unchanged**: reporting a re-save as a changed input is the false positive this exists to remove, and `Comparison.changed` already returns False for it. A signal absent on either side is reported as unavailable and never counted as agreement. |
